  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium ${isCritical ? 'text-destructive' : 'text-warning-foreground'}`}>
          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium mr-2 ${
            warning.scenario === 'rent' ? 'bg-info/10 text-info' : 'bg-success/10 text-success'
          }`}>
            {scenarioLabel}
          </span>
          {warning.message}
...
export function FeasibilityWarnings() {
  ...
  return (
    <div className={`rounded-xl p-4 ${
      hasCritical ? 'bg-destructive/5 border border-destructive/20' : 'bg-warning/5 border border-warning/20'
    }`}>
      <div className="flex items-start">
        ...
        <div className="ml-3 flex-1">
          <h3 className={`text-sm font-medium ${hasCritical ? 'text-destructive' : 'text-foreground'}`}>
